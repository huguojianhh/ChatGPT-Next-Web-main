...
export class DifyaiApi implements LLMApi {
  ...
  async chat(options: ChatOptions) {
    ...
    const modelConfig = {
      ...
      ...{
        difyConversationId: useChatStore.getState().currentSession()
          .difyConversationId,
      },
    };
    ...
    if (modelConfig.difyConversationId) {
      conversation_id = modelConfig.difyConversationId;
    }

    const requestPayload: DifyaiChatRequest = {
      query: lastInput,
      inputs: {},
      response_mode: shouldStream ? "streaming" : "blocking",
      conversation_id,
      user: useAccessStore.getState().userId,
    };

    console.log("[Request] openai payload: ", requestPayload);

    try {
      let chatPath = this.path(Difyai.ChatPath);
      const chatPayload = {
        method: "POST",
          ...
          onmessage(msg) {
            try {
              ...
              if (!modelConfig.difyConversationId) {
                ...
                modelConfig.difyConversationId = conversation_id;
              }

              if (event === "message_end" || finished) {
                return finish();
              }
  ...
  path(path: string): string {
    ...
    if (baseUrl.trim().length === 0) {
      ...
      baseUrl = isApp ? DEFAULT_API_HOST + "/api/proxy/Difyai" : ApiPath.Difyai;
    }
